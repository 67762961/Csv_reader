...
Para_mode.dvdtmode      = [80, 20, 10, 90];                 %% dvdt模式选择     新设定dvdt计算起始和结束百分比
Para_mode.didtmode      = [10, 90, 80, 20];                 %% didt模式选择     新设定didt计算起始和结束百分比
Para_mode.d_dtstep      = [10,10];                           %% 极限d_dt计算的窗口长度 输入时间/ns 会自动近似到采样间隔倍数 第一个是didt 第二个是dvdt
...
Para_mode.I_meature     = "Id";                             %% 以 Ic 或 Id 或 I_fuzai 计算的实际测试电流值
...
Para_mode.CSV_mode      = "双脉冲";                         %% CSV波形类型 影响文件读取方式和过零点判断  特殊模式 "窄脉宽" "窄续流" "窄脉冲"
Para_mode.NameStyle     = "泰克";                           %% 文件命名风格     横河 或 泰克 或 麦克信
