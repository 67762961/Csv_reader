...
- #### 文件配置参数
    ...
    - ##### Para_file.datnum
        &emsp; &emsp;该组数据的数量，程序将从起始编号开始读取该数量个数据为一组，正整数类型。若仅看单个CSV文件，此项填1；多个SCV文件，按照文件数量填写即可。后改版为可以填稍大值，若填的值未找到，将会搜寻更小值。例如文件夹里仅有000到016无017，填入20时可自动定位到016为最末尾文件，但017到020必须为空。
- #### 模式配置参数
    ...
    - ##### Para_mode.Fuzaimode
        &emsp; &emsp;负载电流模式开关兼通道配置，非负整型。若填值不为0，则打开负载电流模式。填的数值+1则为负载电流数据在CSV文件中的列数。~~开启负载电流模式后，将无法计算损耗以及其他跟电流相关的数据，仅分析尖峰。~~ 开启负载模式后，负载电流同时也会出现在 Ic Id 的输出图像中，同时可以选取 I_fuzai 为电流top计算凭据。
    - ##### Para_mode.INTG_I2t 
    ...
    - ##### Para_mode.I_meature
        &emsp; &emsp;电流top计算来源，字符串哈希表。仅"Ic"、和"Id"有效，若填入"Ic"则是使用寻找关断时刻最大电流计算Ictop；若填入"Id"则是通过续流电流的静态范围均值计算Ictop。实际使用时，若实验时采用Ic确定实际电流则填入"Ic"；若实验时采用Id确定实际电流则填入"Id"。若开启负载模式也可以填入 "I_fuzai"
            &emsp; &emsp;示例: "Id"、"Ic"、"I_fuzai"
    - ##### Para_mode.gate_didt
    ...
    - ##### Prra_draw.CSV_mode
        &emsp; &emsp;波形类型选择，字符串类型。用于设定不同的开关处识别风格，例如"双脉冲"模式下，将识别第二次关断和第三次开通；"窄脉冲"模式将分两次读数，先识别二次关断三次开通，再识别一次关断二次开通，共读两遍。
        &emsp; &emsp;示例: '双脉冲'、'窄脉冲'
    - ##### Para_mode.NameStyles
...
- #### 电流校准
    ...
    **注：v2.4.5版本后Ic选取位置改为(toff2 + 关断时长/2 , ton3 - 关断时长/4)，即原标记处的右半边。**
 ...
 - #### Ictop & Icmax 计算
    ...
    - 绿色标记中间为负载电流计算区间。
    &emsp;
    ![电流输出图像](<Readme/计算过程绘图/带负载电流输出图像.png>)
    &emsp;
...
- #### Eon & Eoff 计算
    ...
    &emsp;
    ##### 输出图像：
    &emsp;&emsp;在输出图像中，~~ 所有曲线都是经过了归一化，纵坐标量纲并无实际意义。~~ 目前所有电流电压曲线都分左右坐标数值化，同时实现开关纵坐标统一；Pon 和 Poff 曲线虽然经过了归一化，但同样进行了开关纵坐标统一。
    其中:
    - 红色标记为电流电压相乘的功率曲线；
    - 实线部分为计算积分面积的功率曲线；
    - 虚线部分为未计入积分计算的部分；
...
- #### 报错过零点识别错误，应当如何解决？
    1. 首先，报此错误时，会同时弹出一个绘图窗口，输出门极波形，并标注出检测到的过零点。
    2. 在确认输出波形的确是门极波形后，可以滚轮放大、缩小查看标记的点位到底是哪些地方误判了。（如下图，几个关断位置都有多个红圈标记，表明多个点位误判断）
    ![过零点异常弹出图](Readme/常见问题绘图/过零点异常弹出图.png)
    3. 放大后可以看到，误判都是因为门极波形毛刺引起。（为了表现效果调整了参数，实际运行时该等程度的毛刺并不会报错）。
    ![过零点异常弹出图放大](Readme/常见问题绘图/过零点异常弹出图放大.png)
    4. 解决方案1，调整调整参数 Para_mode.Smooth_Win ，该参数第一数表示对门极波形进行几个点的滤波，初始一般为1，即不滤波。可以适当上调以解决过零点异常问题，通常改10或20可以极大程度缓解，不推荐50或以上的值。
    5. 解决方案2，调整参数 Para_mode.Vgeth ，虽然该参数设计本意是准确填入 Vth ，但在算法迭代后，已经会通过分段自动判断区间，因此并不必要将准确的 Vth 填入。同时，如果遇到过零点异常问题，可以尝试结合异常图更改该参数以正确识别门极电压上升下降区间。