...
% 输出数据配置
Para_out.titlemode = '2Duiguan';                                          %% Full Standard 2Duiguan Manual
Para_out.title_Manual = {'脉宽长(us)', '  CSV  ', 'Ic(A)', 'Eon(mJ)', 'Eoff(mJ)', 'VceMAX(V)', 'VdMAX(V)', 'Vcetop(V)', 'dv/dt(V/us)', 'di/dt(A/us)', 'Erec(mJ)', 'Prrmax(kW)', 'Vgedg1max(V)', 'Vgedg1min(V)',  'Tdon(ns)', 'Tdoff(ns)', '    ','    ','    '};
