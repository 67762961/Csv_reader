...
%% 参数配置
Para_file.location   = 'D:\_Du_chengzhi\Csv_reader\TestLib\20250403';                       %% 读取文件夹       文件夹以八位日期命名
Para_file.tablename  = 'INVB';                              %% csv文件名        一般为BOOSTH(L) 或 INVA(BC)
Para_file.dataname   = '320英飞凌-BT1-常温-600-内管加电容（复测）';     %% 输出数据标签     (机型)-(器件)-(温度)-(电压)-(Ron)-(Roff)
...
Para_mode.dvdtmode   = [30, 80];                            %% dvdt模式选择     额外的dvdt计算起始和结束百分比
