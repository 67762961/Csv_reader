...
%% 参数配置
Para_file.location   = 'E:\20250818';                               %% 读取文件夹       文件夹以八位日期命名
Para_file.tablename  = 'INVC';                                      %% csv文件名        一般为BOOSTH(L) 或 INVA(BC)
Para_file.dataname   = '赤霄-AT5-高温-1150V';                        %% 输出数据标签     (机型)-(器件)-(温度)-(电压)-(Ron)-(Roff)
Para_file.datstart   = 020;                                         %% csv序号起始点    该工况的首个CSV文件序号
Para_file.datnum     = 14;                                          %% 本组csv文件      该工况一共测试组数 BOOST 12 INV 16

% 模式配置参数    
Para_mode.nspd       = 1.6;                                         %% csv采样率        CSV保存时示波器设置ns/pt
Para_mode.Chmode     = 'setch';                                     %% 通道分配模式     'setch'或'findch'
Para_mode.Ch_labels  = [1, 2, 5, 4, 3, 6];                          %% 通道分配         Vge Vce Ic Vd Id对应通道
...
Para_data.gate_Erec  = 50;                                          %% Erec抬升容错     Erec计算中下降沿过滤毛刺阈值
