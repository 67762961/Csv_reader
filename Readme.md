...
- #### 模式配置参数
    ...
    - ##### Para_mode.didtmode
        &emsp; &emsp;电流didt计算区间，正整数数组型。共四个参数，与dvdt相同，前来两个参数为开通didt的选点，一般的应当为 10, 90 ，后两个为关didt的选点，一般取 90, 10。这与dvdt计算区间设置原理相同，但不同的是，理论上didt的计算应当就是10%到90%Ictop，原则上不应当设置别的数值。但未防止状态机识别起始算法失效以及其他原因产生的报错影响读取，所以保留此接口。同时，与dvdt计算区间参数不同的是，手动设置计算区间后，输出将会变为只有手动设置段。PS：极端条件下，结束位置可以取到高于100%，但极度不推荐此参数。
            &emsp; &emsp;示例: 
    ...
    - ##### Para_mode.I_meature
        &emsp; &emsp;电流top计算来源，字符串哈希表。仅"Ic"、和"Id"有效，若填入"Ic"则是使用寻找关断时刻最大电流计算Ictop；若填入"Id"则是通过续流电流的静态范围均值计算Ictop。实际使用时，若实验时采用Ic确定实际电流则填入"Ic"；若实验室采用Id确定实际电流则填入"Id"。
            &emsp; &emsp;示例: "Id"、"Ic"
- #### 数据配置参数
    - ##### Para_data.gate_didt
        &emsp; &emsp;开通(关断)电流didt上升(下降)容错，非负整数类型，可填0。第一个参数为didt上升沿检测算法的检测窗口长度，若在i到i+gate_didt的区间内呈上升趋势则判断为上升；反之则判断为下降重置初始点。填0时即为设置成要求必须严格上升，即i+1的数值必须比i高。第二个参数为下降didt的对应参数，原理一致。实际使用时，若Ic数据线非常光滑，则可以填0关闭容错；若略有毛刺则可以填值3到5；不建议填30以上的值，下降容错则一般需要填大一点，大致10到30为宜。
...
- #### Eon & Eoff 计算
    ...
    - 在上述 2% Vcetop 识别不到时 自动改为 3%、4% 依次上涨 1%
    - ###### Eoff范围选取
    - 关断时Vce上升阶段中的 10% Vcetop 为关断起点
    - 关断时Ic下降阶段 2% Ictop 为关断终点
    - 在上述关断起点 往前扩充 30% 的关断长度 为损耗计算长度
    - 在上述 2% Ictop 识别不到时 自动改为 3%、4% 依次上涨 1%
    ##### 关于范围选取
    &emsp;&emsp;在是否要在原选取范围中向前扩充 30% 的结果中对比如下图，左侧为未向前扩充的计算结果；右侧为向前扩充 30% 的结果。因为实际运用中，更小的寻找点在实际波形中受到震荡影响太过剧烈，而 10% 的选取点又会使得损耗计算漏掉 10% 之前的损耗部分。因此最后决定在前 10% 后 2% 的基础上，将计算长度向前扩充 30% ，而实际上加入扩充或者不加入扩充对损耗计算的影响也仅 1% 到 2% 。
    &emsp;
...
- #### dvdt & didt 计算
    &emsp;&emsp;计算关断时刻dvdt和开通时刻的didt以衡量开关速度。
    ##### 计算过程
    - 选定其实和结束点；
    - 计算出起始和结束点之间的电压/电流差和时间差；
    - 相除计算得出dvdt或者didt。
    ##### 范围选取
    - 在常规情况下选取的都是 10% 到 90% 的上升区间；
    - 在 dvdtmode 和 didtmode 有特殊设置时按照设置位置寻点；
    ##### 输出图像：
    &emsp;
    ![开关速度计算图像](Readme/计算过程绘图/开关速度计算图像.png)
    &emsp;
